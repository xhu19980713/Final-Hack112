...
def sePoster(path):
    ...
    for key in dic:
        x = dic[key][0][1][0]
        y = dic[key][0][2][1]
        if x >= minX and x <= minX+midX and y >=minY and y <= minY+midY:
            pic1 += [key]
        elif x >= minX+midX and x <= maxX and y >= minY and y <= minY+midY:
            pic2 += [key]
        elif x >= minX and x <= minX+midX and y >= minY+midY and y <= maxY:
            pic3 += [key]
        elif x >= minX+midX and x <= maxX and y >= minY+midY and y <= maxY:
            pic4 += [key]
    return [pic1,pic2,pic3,pic4[1:]]

def interestFilterFour(self,path):
