...
def getTextOne(path):
    ...
    for speci in info.split('\n'):
        ...
        'AM' not in speci and 'PM' not in speci and ':' not in speci:
            infoSep['event'] += [speci]
        elif (',' in speci or 'Ave' in speci or 'Res' in speci or 'Ro' in speci) and 'www.' not in speci or (len(speci) >=2 and speci[0].isupper() and speci[1].isdigit()):
...
# def writeOut(path):
#     dic = getText(path)
#     infoGet = open('infoGet.csv','w',newline = '')
#     writer = csv.writer(infoGet)
#     for key in dic:
#         writer.writerow([key,dic[key][0]])
#     infoGet.close()
